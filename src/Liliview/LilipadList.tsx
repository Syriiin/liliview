...
const LilipadList = (props: LilipadListProps) => {
    ...
    const [selectedLilipad, setSelectedLilipad] = useState<LilipadInfo | null>(
        null
    );
    ...
    const handleNewLilipad = () => {
        async function request() {
            if (
                isSaved() ||
                window.confirm(
                    "Are you sure you want to close the current lilipad?\nUnsaved changes will be lost."
                )
            ) {
                const lilipad = await createLilipad(
                    prompt("lilipad name") || "lilipad"
                );
                if (lilipad !== null) {
                    setLilipads([...lilipads, lilipad]);
    ...
    const isSaved = () =>
        editingLilipad === null || editingLilipad.text === text;
    ...
    return (
        <Container fluid>
            <Row className="vh-100">
                <Col
                    xs={2}
                    className="g-0 d-flex flex-column justify-content-between"
                >
                    <div>
                        ...
                        <ListGroup className="mt-3" variant="flush">
                            {lilipads.map((lilipad) => (
                                <ListGroup.Item
                                    active={lilipad.id === editingLilipad?.id}
                                    key={lilipad.id}
                                    action
                                    as="button"
                                    onClick={() => switchLilipad(lilipad)}
                                >
                                    {lilipad.name}
                                    {lilipad.id === editingLilipad?.id &&
                                        !isSaved() &&
                                        " - UNSAVED"}
                                </ListGroup.Item>
                            ))}
                        ...
                        <div className="m-3 d-flex justify-content-evenly">
                            <Button variant="info" onClick={handleNewLilipad}>
                                new lilipad
                            </Button>
                            {selectedLilipad !== null && (
                                <>
                                    <Button onClick={handleSaveLilipad}>
                                        save
                                    </Button>
                                    <Button
                                        variant="danger"
                                        onClick={() =>
                                            handleDeleteLilipad(selectedLilipad)
                                        }
                                    >
                                        delete
                                    </Button>
                ...
                <Col xs={10} className="g-0 overflow-hidden">
                    {loading && <h3 className="m-5">Loading...</h3>}
                    {editingLilipad !== null && (
